...
def reposition(img,ratio = 0.08):
    ...
    valid_idx_0 = valid_idx[0] + np.random.normal(0, 1, (valid_idx[0].shape[0])).astype(int)
    valid_idx_1 = valid_idx[1] + np.random.normal(0, 1, (valid_idx[1].shape[0])).astype(int)
...
def add_blocknoise(img):
    ...
    return img
def color_jet(image):
    """ this function randomly adjusts the color intensity of each channel in an image. 
    The adjustment is done by raising the pixel values to a random power determined 
    by the scale factor for each channel. The purpose is to introduce random 
    color variations to the image."""
    scale = None
    for n_c in range(3):
        if scale is None:
...
def defineHighlightArea(H,W,coord_map):
    """generates a mask with highlighted areas in the shape of an ellipse. 
    The parameters of the ellipse are randomly determined, 
    resulting in a unique highlight pattern each time the function is called."""
    mask = np.zeros((H*W,3))
...
def defineCoorMap(H,W):
    """
    generate a coordinate map for a 2D grid with dimensions H and W
    pair of x and y coordinates --> 2D array containing all these coordinate pairs
    """
